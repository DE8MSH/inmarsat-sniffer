...
void basestation_send_position(const char *reg, const char *flight,
                                ...
                                int alt_ft, double gs_knots_in,
                                double track_deg_in, uint64_t timestamp_ns)
{
    ...
    /* Use provided heading/speed if valid, else derive from consecutive fixes.
     * ADS-C tag 14/15 provides both directly when present. */
    double gs_knots = 0, track_deg = 0;
    int have_track = 0;
    if (gs_knots_in >= 0 && track_deg_in >= 0) {
        gs_knots = gs_knots_in;
        track_deg = track_deg_in;
        have_track = 1;
        /* Still update the track cache so future derived calculations
         * have a current reference point */
        bs_derive_track(hex, lat, lon, timestamp_ns, &gs_knots_in, &track_deg_in);
    } else {
        have_track = bs_derive_track(hex, lat, lon, timestamp_ns,
                                      &gs_knots, &track_deg);
    }
